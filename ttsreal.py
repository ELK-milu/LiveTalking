...
class VoitsTTS(BaseTTS):
    ...
    def txt_to_audio(self,msg):
        ...
        self.stream_tts(
            self.gpt_sovits(
                text=text,
                reffile=str(request['reffile']),
                reftext=str(request['reftext']),
...
class VolcanoTTS(BaseTTS):
    ...
    def doubao_voice(self, request) -> Iterator[bytes]:
        start = time.perf_counter()
        text = request['text']
        voice_type = request['reffile']
        # 创建请求对象
        post = self.Post = self.PostChat(self.appid, self.cluster,self.SESSION)
...
class TencentTTS(BaseTTS):
    def __init__(self, opt, parent):
        super().__init__(opt,parent)
        self.appid = self.parent.config["TTS"]["TencentTTS"]["appid"]
        self.secret_key = self.parent.config["TTS"]["TencentTTS"]["secret_key"]
        self.secret_id = self.parent.config["TTS"]["TencentTTS"]["secret_id"]
        self.voice_type = int(self.parent.config["TTS"]["TencentTTS"]["voice"])
        self.codec = "pcm"
        self.sample_rate = 16000
        self.volume = 0
        self.speed = 0
